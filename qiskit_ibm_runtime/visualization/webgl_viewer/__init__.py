...
"""Python bridge that produces a self-contained WebGL2 HTML viewer for circuit schedule timing."""
...
def draw_circuit_schedule_timing_webgl(
    ...
    """Return a self-contained HTML string for the interactive WebGL2 circuit schedule viewer.

    The viewer is rendered entirely in the browser using WebGL2 — no Python runtime
    dependency after this function has been called.  All 100K+ instructions are drawn
    in a single GPU instanced-draw call, delivering buttery-smooth pan and zoom.

    **WebGL2 is required.** All modern browsers (Chrome, Edge, Firefox, Safari 15+)
    support WebGL2.  There is no Canvas 2D fallback; an error banner is shown
    instead when the context is unavailable.

    Interactive features include mouse-wheel zoom on the x-axis (Shift+Wheel for y),
    drag-to-zoom-to-region, Shift+drag to pan, double-click to reset the view, a
    collapsible sidebar with per-gate / per-channel / per-branch visibility toggles,
    filter buttons, a text search box with jump-to-next-hit, and an overview minimap.
    Text labels inside rectangles are rendered using a runtime-built SDF (signed
    distance field) atlas so they remain crisp at any zoom level.

    This function is a high-performance alternative to
    :func:`~qiskit_ibm_runtime.visualization.draw_circuit_schedule_timing_html`,
    which uses Canvas 2D and may become laggy at 100K+ instructions.
    ...
    Example::
        ...
        from qiskit_ibm_runtime.visualization import draw_circuit_schedule_timing_webgl

        timing = result[0].metadata.scheduler_timing.timing
        html = draw_circuit_schedule_timing_webgl(
            timing,
    ...
    # Build the JS options object (only include non-default values)
    js_options: dict = {}
    ...
    pkg = importlib.resources.files(__name__.rsplit(".", 1)[0] + ".webgl_viewer")
